...
def createproduct(request):
    ...
    if request.is_ajax():  # nếu như yêu cầu nhận được là ajax
        ...
        if formProduct.is_valid() and formVariants.is_valid():
            ...
            if 'image' in request.FILES:  # lưa hình ảnh
                product.image = request.FILES['image']
    ...
    return render(request, 'product/createproduct.html', {'formProduct': ProductForm(prefix="productForm"), 'variantsForm': VariantsForm(prefix="variantsForm"),'ImageProductForm':ImageProductForm(prefix="ImageProductForm") ,'categorys': categorys, 'sizes': sizes, 'colors': colors, 'imageProductList': imageProductList})
